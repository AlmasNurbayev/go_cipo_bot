...
**Config.** `config.Mustload(path)` — godotenv (если передан `-configEnv`) + `caarlos0/env`. Секреты помечены `json:"-"`, потому что конфиг печатается в лог при старте. Изменяемые бизнес-настройки лежат не в env, а в таблице `app_settings` (JSON-значения) и читаются хелперами `config.GetSettingsString/Float64/Int/USDRates/GSheetsSources` — так задаются статьи ОПиУ, курсы доллара, ссылки на Google Sheets, максимальный период списков чеков (`SUMMARY_CHECKS_MAX_DAYS`). `GetSettingsInt` в отличие от остальных не возвращает ошибку, а отдаёт переданное значение по умолчанию — отсутствие строки в таблице не должно ронять фичу. Настройки читаются из БД на каждый запрос, поэтому правка значения действует без перезапуска бота.
...
- Курсор рассылки — колонка `users.transaction_cursor`; `DetectNewOperations` сдвигает его всегда, даже если сообщение пользователю по фильтру роли не отправляется. Поэтому опубликованное в NATS сообщение — единственный шанс доставить чек, второй раз апдейтер его не пришлёт.
- Контракт подтверждения в `RunNatsConsumer`: `Ack` только после реальной отправки в Telegram либо после ошибки, которую повтор не исправит (403/400/404 — `isPermanentTelegramError`). Всё остальное, включая неизвестные ошибки и 5xx, — `NakWithDelay`, переотправляет брокер. `SendToNats` публикует по одному сообщению на транзакцию с `nats.MsgId`, чтобы Nak не приводил к повторам уже доставленных чеков.
- Консьюмер `bot_consumer` настраивается явно в `ensureConsumer` и подключается через `nats.Bind`. Через `PullSubscribe` с durable-именем подключаться нельзя: тогда подписка сама создаёт консьюмер, а `Unsubscribe` удаляет его вместе с курсором доставки.
