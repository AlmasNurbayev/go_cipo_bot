# GO_CIPO_BOT

Суть - хранилище данных продаж / остатков товара / финансовых итогов магазина Cipo.
Интерфейс пользователя реализуется через telegram-бот.

Описание всех команд и кнопок бота — [COMMANDS.md](COMMANDS.md).

# Источники данных / API

- продажи и аналитика продаж - чеки с API ОФД, недокументированный RestAPI
- остатки товаров - бэкенд сайта Cipo, который их получает из 1С Розница, типовой обмен с сайтом в CommerceML
- отчет о доходох и расходах - API Google Sheets, таблица с расходами, себестоимостью

# Основные сервисы

- Postgres в стандартном контейнере
- KOFD_UPDATER - парсер данных с внешнего API KOFD и сохранение в БД
- BOT - интерфейс для доступа к данным в виде телеграм-бота
- Nats - брокер сообщений в стандартном контейнере, между KOFD_UPDATER и BOT

# Пакеты планируемые для использования

- [go-telegram] (github.com/go-telegram/bot)
- pgx / pgxpool / scany
- github.com/go-analyze/charts
- github.com/golang-migrate/migrate

# TODO
- [v] 2026-08-14 - BOT/summary - обе кнопки "все чеки" доступны только за короткий период, лимит в app_settings.SUMMARY_CHECKS_MAX_DAYS (миграция 013, по умолчанию 7 дней, читается на каждый запрос - правка в БД действует без перезапуска). На месяце/годе кнопки не показываются (utils.IsPeriodWithinDays), а callback из старого сообщения отбивается текстом с актуальным числом дней. Лимиты Telegram (4096/1024/32768 символов, 8 кнопок в ряду, пауза 400 мс) остались константами в коде - они диктуются протоколом, а не бизнесом. Внутри дня список больше не отказывает на длинном тексте - режется на части по 4096 символов с раздачей inline-кнопок по своим сообщениям. Длина считается в кодовых единицах UTF-16, как её считает Telegram (кириллица - 1 единица на символ, то есть 4096 русских букв проходят целиком), утилиты TelegramLen/TrimToTelegramLimit в internal/lib/utils/telegram_limits.go. Заодно исправлен лимит подписи к медиагруппе (был 1096, у ботов 1024) - вместо отказа теперь обрезка по строкам, чтобы не рвать HTML-теги
- [v] 2026-08-14 - BOT/summary - кнопка "Все чеки #": таблица (№ / дата - время / тип / сумма / в т.ч. карта / в т.ч. нал) через sendRichMessage (Bot API 10.1), лимит текста там 32768 вместо 4096. Разметка - GFM pipe-таблица в поле markdown у InputRichMessage. Таблица режется по 40 строк на сообщение, к каждой части свои кнопки-номера для открытия чека (5 рядов по 8) - не текстом упирается, а тем, что сотня кнопок под одним сообщением нечитаема и близка к пределу размера reply_markup. Нал/карта берутся напрямую из колонок transactions.sum_card/sum_cash, в cheque_json не заходим; у выемок/внесений прочерк, у операций старее 2026-08-03 колонки пустые и считаются нулём. Если сервер не знает rich-сообщений, отправляется та же таблица моноширинным блоком с разбивкой по 4096
- [v] 2026-08-14 - KOFD_updater - GetGoodsFromCheque: скидка применялась только к первому из нескольких одинаковых товаров в чеке (поиск позиции возвращал первое совпадение по имени+размеру, вторая строка скидки попадала в ту же позицию). Теперь каждая строка скидки/наценки ложится на свою, ещё не обработанную позицию; наценка сопоставляется так же, как скидка (нормализация пробелов + размер). Уже сохранённые в БД чеки не пересчитываются - InsertTransactions делает ON CONFLICT DO NOTHING. Добавлены первые тесты пакета utils, make test теперь запускает ./cmd/... ./internal/...
- [v] 2026-08-13 - BOT/KOFD_updater - надёжная доставка уведомлений: консьюмер подтверждает сообщение NATS только после реальной отправки в Telegram, временные сбои (сеть, 5xx, 429) уходят в NakWithDelay 30 сек и переотправляются брокером, постоянные (403/400/404) подтверждаются с записью в лог. Консьюмер bot_consumer настраивается явно (AckWait 5 мин, MaxDeliver -1) и подключается через nats.Bind - раньше PullSubscribe с durable-именем создавал консьюмер сам, а defer sub.Unsubscribe() удалял его при остановке вместе с курсором доставки. Апдейтер публикует по одному сообщению на транзакцию с Nats-Msg-Id для дедупликации
- [v] 2026-08-03 - KOFD_updater/BOT - разбивка суммы смешанной оплаты на нал/карту: sum_cash/sum_card в transactions (парсинг текста чека), учтено в аналитике summary/finance. В списках чеков и NATS-уведомлениях о новых операциях по-прежнему выводится пометка "Смешанно" (GetTypePaymentText не менялся) - см. docs/payment-split.md
- [v] 2026-07-30 - other - кнопка отправки тестового сообщения выбранному пользователю с отчётом о статусе
- [v] 2026-07-26 - обновить пакет бота до релиза https://github.com/go-telegram/bot/releases/tag/v1.22.0 (Bot API 10.1, Rich Messages) - ломающих изменений нет, новый API не используется
- [v] 2026-07-26 - обновить Go до 1.26 (go.mod, Dockerfile), базовый образ alpine до 3.24.1, в CI добавить actions/setup-go@v6 перед линтом, в Dockerfile копировать go.sum в слой зависимостей
- [v] обновить пакет бота до релиза https://github.com/go-telegram/bot/releases/tag/v1.21.0
- [v] parserJSON - добавить загрузку закупочных цен, также добавить из 1С
- [v] обновить экшны - actions/checkout@v6, docker/build-push-action@v7, docker/login-action@v4, docker/setup-buildx-action@v4, golangci/golangci-lint-action@v9
- [v] 2026-01-20 - в блоке "Charts" сделать кнопку "12m" чтобы на 12 месяцев назад выводить итоги, блоками по 2 колонки (январь 2026 и январь 2025, февраль 2026 и февраль 2025 и т.д.)
- [ ] 2026-01-19 - в блоке "Остатки" сделать кнопку "Диаграмма" чтобы выводить гистограмму с суммами остатков каждый месяц (всего 12 колонок для 12 месяцев), внутри разделить по сезонам
- [ ] 2026-01-02 - сделать бинарник webkassa_updater для загрузки транзакций из Вебкассы.
      Использовать метод для авторизации - POST https://my.webkassa.kz/api/authorize/spa/email
      Использовать метод для получения смен - GET https://my.webkassa.kz/api-history/v3/Reports/HistoryCheckShiftGrid?cashboxId=426032&skip=0&take=20
      Использовать метод для получения данных чека - POST https://my.webkassa.kz/api/v4/Check/HistoryByNumber
      Для методов после авторизации требуются 1 кука с AccessToken

- [v] 2025-10-19 - summary - в аналитику добавить процент возвратов
- [ ] 2025-10-16 - финансы - диаграмма - вывести четкий ноль и значения в разные стороны
- [ ] 2025-10-04 - финансы - добавить режим сравнения произвольных периодов
- [v] 2025-05-14 получили токен, теперь надо получить операции
- [v] 2025-05-15 записываем в БД транзакции полученные из КОФД, но нужно получить и записать в БД информацию из чека, названия, путь до картинки
- [v] 2025-07-19 заменить пакет для telegram api на более документированный https://github.com/go-telegram/bot
- [v] 2025-07-19 kofd_updater вставку операций делать обернуть в транзакцию Postrges, транзакцию создать в его main.go
- [v] в GetGoodsFromCheque - отпарсить из чека не только имена, но и кол-во, цены
- [v] поле cheque в БД поменять тип на JSON или связанную таблицу "Товары в транзакциях"
- [v] 2027-07-24 - BOT - запросить URL-картинки по name_1c к бэкенду сайта. Открытый вопрос - хранить ли URL в базе данных, так как 1С может менять имя файла
- [v] 2027-07-24 поле cheque в БД - также парсить секцию Скидки, добавить поля в cheque_json
- [v] 2025-07-20 - BOT - вывод по кнопке "Все чеки" по итогам дня или недели
- [v] 2025-07-26 - KOFD_updater - при каждом обновлении помещать новые операции в Кафку/RMQ
- [v] 2025-07-26 - BOT - читать новые операции из Кафки/RMQ
- [v] 2025-08-09 - BOT - выводить отдельные чеки с картинками
- [v] 2025-08-09 - BOT - в GetPeriodByMode добавить вариант получения даты по шаблону "2025 08 21", для получения итогов по любому дню
- [v] 2025-08-09 - KOFD_updater - заменить публикацию Kafka на Nats
- [v] 2025-08-12 - BOT - заменить слушателя Kafka на Nats
- [v] 2025-08-10 - если в сервисе getAllChecks сообщение превысит лимит в 4096 символа, то нужно разбить на части, а не отказывать. Но суммарно не более 50 чеков
- [v] 2025-08-10 - добавить функцию по выводу типа оплаты и добавить в список всех чеков и в одиночный чек
- [V] 2025-08-11 - деплой через github actions - сборка контейнера с бинарниками после линта, пуша тега
- [V] 2025-08-11 - docker compose - закрыть клиентский порт для наружнего доступа
- [V] 2025-08-11 - в Cron повесить запуск updater каждые 2 мин
- [v] 2025-08-11 - создать в БД вторую уже закрытую кассу SWK00426370 - 010102360873 - Евразия
- [v] 2025-08-11 - залить все чеки с 01.07.2021 по 4 кассам, апдейтером, кусками по 1-3 мес. Чтобы не создавать множественную нагрузку по запросам чеков.
- [v] 2025-08-17 - добавить в итоги текущего дня остаток по кассе
- [v] 2025-08-16 - диаграммы (использовать новый форк go-chart от https://github.com/go-analyze/charts)
- [v] 2025-08-16 - финансы - сделано 2025-10-04
- [ ] 2025-08-31 - подумать как сделать кнопку "Полный текст чека" для медиагруппы удобнее, сейчас приходится отправлять вторым сообщением
- [v] разобрать сумму скидок в аналитике за 2025-09-04, скидки превысили сумму продаж
- [v] 2025-10-04 - финансы - разобраться с шириной и высотой при разном кол-ве строк/колонок
